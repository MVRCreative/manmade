...
test.describe('Sanity', () => {
  test.describe('Static pages', () => {
    test('should display the homepage', async ({ page }) => {
      ...
      await expect(
        page.getByRole('heading', { name: 'Canon Plus Landing Page Boilerplate' }),
      ).toBeVisible();
    });

    test('should navigate to the design system page', async ({ page }) => {
      await page.goto('/');

      await page
        .getByRole('link', { name: /View Design System/u })
        .first()
        .click();

      await expect(page).toHaveURL(/design-system$/u);
